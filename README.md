...
6. 初赛结束，以B榜成绩作为初赛成绩依照，要求TOP30团队提交代码审核，代码提交截止时间11月30日12:00。组委会将审核并取消存在人工标注、相互抄袭等行为队伍的比赛资格，晋级空缺名额后补。初赛成绩符合要求且通过实名认证的排名前20名的参赛队伍将进入复赛。

**复赛**（2022.12.1-2022.12.31）

1.复赛阶段不再做模型训练，测试数据仍使用B榜数据集。

2.参赛选手使用算能SDK编译初赛训练好的模型，进行模型移植。模型转换在本地完成，推理测试环境可申请算能云开发空间：https://cloud.sophgo.com/developer/platform/index

3.复赛评测：复赛以在算能TPU平台上推理的精度和性能作为评测标准。参赛者对测试集图片进行推理后，将测试集图片的检测结果和推理时间(ms)以txt文件保存，并上传至result_submit/文件夹下，上传格式实例如下所示：

```
<class_name> <confidence> <left> <top> <right> <bottom> <time_cost_ms>
```

4.评分方式：

- 通过与初赛相同的mAP(mean Average Precison)指标评测模型精度。
- 通过模型推理时间i_time评测模型性能，i_time为测试集图片推理的平均时间，单位为ms。
- 最终得分计算公式为：score=mAP×100+(1000-i_time)×0.1，分数高者为优。

- 排行榜每日更新1次。

5.复赛结束，参赛者需提交bmodel，由组委会统一测试最终成绩。

注：复赛成绩换算公式=(x-xmin) / (xmax-xmin)，其中x为原始得分，xmin为第30名的得分，xmax为第一名的得分。
...
![image](https://raw.githubusercontent.com/ForestMa/SOPHGO-HVOD-Contest/main/image_QRcode/%E3%80%90AI%E7%AE%97%E6%B3%95%E5%88%9B%E6%96%B0%E8%B5%9B%E3%80%91%E4%BA%A4%E6%B5%81%E7%BE%A4.png)
